...
with models.DAG(
    ...
    get_linha_task = PythonOperator(
        task_id="get_linha",
        python_callable=get_linha
    )

    get_twitter_token_task = PythonOperator(
        task_id="get_twitter_token",
        python_callable=get_twitter_token
    )
    ...
    consume_api_task = SimpleHttpOperator(
        ...
        headers={"Authorization": "Bearer {{ ti.xcom_pull(task_ids='get_twitter_token') }}"}
    )
